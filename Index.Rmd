...
In this portfolio, I will try to analyze the way Rush's music changed over the years. I will try to find certain characteristics across their albums, and see whether I can spot certain eras of albums that share these characteristics. 

Column
---

### The Corpora 
Of course, the most important corpus I will use is that containing the audio features of the entire discography of Rush itself. Upon initial inspection, I decided to remove the album "2112 - 40 Years Closer: A Q&A With Alex Lifeson And Terry Brown Commentary" as it contains commentary and is thus not relevant to my analysis. Furthermore, I will include the more detailed audio analysis for each track in the corpus. 


Initial Data Exploration {.storyboard}
===

### First, I performed an __initial analysis__ of the __basic features__ of the studio albums {data-commentary_width=500}
...
### These features proved promising; so I proceeded to do a __principal component analysis__ of all Rush's albums.
...
We can see that this principal component classifies all live-albums of Rush perfectly. However, upon further inspection, this enormous difference can be fully accounted for by the Spotify feature _liveness_. This is unsurprising as in this plot we're classifying exactly that, but it is a good reminder that a PCA can't be applied blindly to any dataset. 

I will now perform a PCA on only the studio albums.
...
Looking at the first four principal components we can see that indeed the first one captures the most radical changes across albums. Furthermore, it looks like these principal components divide Rush's discography up in about 5 different eras. It's important to remember that each principal component doesn't resemble any feature or property per se, but that you can make an estimation of album similarity based on the _difference_ in principal component value between them. 
The next step will be to perform an actual clustering of this data and evaluate whether we need any more data to cluster the studio albums.

K-means clustering {.storyboard}
===

### __Introduction__, determining __parameters__ for the clustering {data-commentary_width=500}
...
rush_albums_clustered %>%
  group_by(cluster) %>% 
  mutate(name=paste(album_name, collapse='\n')) %>% 
  mutate(count=nrow(album_name)) %>%  
  plot_ly(labels=~paste("Cluster", as.character(cluster)), 
          values=~nrow(.), 
          type='pie',
          ...
          text=~name,
          showlegend=FALSE) %>% 
  layout(title = "Distribution of the album clusters")
```

***

This beautiful pie-chart shows the distribution of clusters across the studio albums (the album names within each cluster are shown on hover). Upon intital inspection I find some of the clusterings rather peculiar, based on the feeling I get from purely listening to the albums. However, before we dive into what I subjectively think, I will first perform a timbral and chordal analysis to see whether my subjective antipathy to this clustering is purely subjective, or there truly is something wrong with it.
...
According to the spotify timbre documentation, c02 roughly correlates with the brightness. I'm inclined to believe that this correlation is inversed, i.e. higher c02 values implies a lower brightness. This is because the cluster with the lowest average brightness, cluster two, consists of two albums in which Rush uses a lot of brighter instruments (for instance, [these](https://open.spotify.com/playlist/35ps4GpYuPuAoiIG6kebAZ?si=dK4GyFCFT4GMZLmnJg7sZg) songs). This is in stark contrast to for instance cluster 4, that have an overall (though subjectively) darker tone (like [these](https://open.spotify.com/playlist/2btSbSR7hE9iHozoK4fG8V?si=2C2pIskmQh-WjFOP1QwiwQ) songs) 

This inverse correlation would also explain the fact that cluster 1 also has a lower c02 value on average. This cluster contains most of the early work of Rush (followed by cluster 2). The singer of Rush is known for a very high pitched voice, however his pitch has lowered through the years quite steeply.
...
Here we can see the distribution of different tempi of songs across the different clusters. We can see that the mean tempo lies in the same range for each cluster, namely around 125-135bpm. However, the mean tempo confidence that the spotify API gives for each cluster varies a lot. Spotify has a lot less confidence in its tempo classifications of the first and second cluster, relative to the third, fourth and fifth cluster. I think this is because the songs in the first, second and to a lesser extent the third cluster have a lot more changes in tempo. This is to be expected, as these clusters contain most of the albums that are commonly seen as the most progressive. 
...
Vapor Trails and Test for Echo were released in line, featuring a very guitar-oriented instrumentation. This also the case for Rush's self-titled, their very first album. I find these three very fitting in one cluster, however I was at first surprised to see Snakes & Arrows here. Snakes & Arrows was the second to last album to be released and it has a very distinctive atmosphere to it. Therefore, I didn't associate it with the other albums in this cluster. However, upon second listening I discovered that Snakes & Arrows indeed has a high focus on guitar as well, albeit with a very different atmosphere. This might be the reason that Snakes & Arrows branches off the highest in this cluster.

__The two outliers__, 2112 and Presto. During my previous exploration I've already noticed Presto being an outlier, however I'm surprised to see 2112 at the very root of the dendrogram. Released between Caress Of Steel and A Farewell To Kings, I would have expected to see it clustered with the first group I discussed. 2112 starts with a twenty minute long [epic](https://open.spotify.com/track/2DMDV9kUEO2WjUHg5acHBY?si=ZUEm2KqdRL6YEtS9MoAPXQ), which is actually seen as one of Rush's all-time masterpieces. This sudden spike in popularity was what they needed to be able to start on a project like A Farewell To Kings. Coincidentally (or is it), Presto is one of Rush lesser-received albums. It is released between the synth/pop-cluster and the cluster consisting of Roll the Bones and Counterparts. I think it can be best described as somewhat of a transition between these clusters. This could explain the fact that the album forms its own cluster and the fact that it branches off lower than 2112, as it still is 'only' a transition from one era to the next instead of something different altogether. 
...
The track I chose is [Xanadu](https://open.spotify.com/track/5q7KiSN7znIONR76LAqm7k) from A Farewell To Kings. I will compare this studio version with [this](https://open.spotify.com/track/3THzmYancb8RVehmqItO6G) live version from the live album Exit... Stage Left.

```{r spotwidgets}
htmltools::includeHTML('html/xanadu_studio.html')
htmltools::includeHTML('html/xanadu_live.html')
```

```{r xanadu_extract}
xanadu_st <- get_tidy_audio_analysis('5q7KiSN7znIONR76LAqm7k') %>%
  select(segments) %>% unnest(segments)
xanadu_live <- get_tidy_audio_analysis('3THzmYancb8RVehmqItO6G') %>% 
  select(segments) %>% unnest(segments)
```
...
xanadu_live_st_plot <- compmus_long_distance(
    xanadu_st %>% select(start, duration, pitches) %>% mutate(pitches = map(pitches, compmus_normalise, 'chebyshev')),
    xanadu_live %>% select(start, duration, pitches) %>% mutate(pitches = map(pitches, compmus_normalise, 'chebyshev')),
    feature = pitches,
    method = 'euclidean') %>% 
    ggplot(
        aes(
            ...
            fill = d)) + 
    geom_tile() +
    scale_fill_continuous(type = 'viridis', guide = 'none', option='inferno') +
...
```{r chroma, eval=TRUE, fig.width=8, fig.align='center'}
...
xanadu_st_tssm <- xanadu_st %>%
  ...
  theme_classic() +
  labs(x = '', y = '') + 
  ggtitle("Xanadu studio")

xanadu_live_tssm <- xanadu_live %>%
  compmus_self_similarity(timbre, 'cosine') %>% 
  ggplot(
      aes(
          x = xstart + xduration / 2, 
...
save(xanadu_st_tssm, file='data/xanadu_st_tssm.RData')
save(xanadu_live_tssm, file='data/xanadu_live_tssm.RData')
```

```{r ssm, fig.width=5, fig.height=5}
load('data/xanadu_st_tssm.RData')
load('data/xanadu_live_tssm.RData')
xanadu_st_tssm
xanadu_live_tssm
```
...
The very first thing that I notice from these two self-similarity matrices, is of course the large bright bars at the left and bottom of the studio matrix. This means that, timbrally, the introduction is very distinct from the rest of the songs. I can imagine that, as discussed before. However it is interesting to see that this distinctiveness isn't visible for the live version of the song. However, a similar introduction is present in the track itself. After listening to the both, I do think the introduction of the live version is a bit quicker, and maybe a bit brighter. 

### __Tempogram__ analysis

```{r, eval=FALSE}
xanadu_st_t <- get_tidy_audio_analysis('5q7KiSN7znIONR76LAqm7k') %>% 
...
xanadu_live_t <- get_tidy_audio_analysis('3THzmYancb8RVehmqItO6G') %>% 
  tempogram(window_size = 8, hop_size = 1, cyclic = TRUE) %>% 
  ggplot(aes(x = time, y = bpm, fill = power)) + 
  geom_raster() + 
  scale_fill_viridis_c(guide = 'none', option='magma') +
...
get_tidy_audio_analysis('2DMDV9kUEO2WjUHg5acHBY') %>% 
  tempogram(window_size = 8, hop_size = 1, cyclic = TRUE) %>% 
  ggplot(aes(x = time, y = bpm, fill = power)) + 
  geom_raster() + 
  scale_fill_viridis_c(guide = 'none', option='magma') +
...
save(xanadu_st_t, file="data/xanadu_st_t.RData")
save(xanadu_live_t, file="data/xanadu_live_t.RData")
```
...
xanadu_st_t + ggtitle("Tempogram Xanadu studio")
xanadu_live_t + ggtitle("Tempogram Xanadu live")
```

***

The tempograms that come from these songs are very interesting. Firstly, both plots do have the same global pattern, however the bright line depicting the most probable tempo is firstly a lot more shaky, which could mean that they 'float' more loosely around the intended tempo to keep the song interesting and challenging to play. The shakiness could however also be due partly by the fact that the music is a little bit bloated with for instance the shouting of the audience. Secondly, the more general, intended tempo also changes more often than in the studio version (see, for instance, the parts between ~200 and ~400 seconds). In both versions, we see three darker areas with a very clear bright line from [150s](https://youtu.be/5w3s2T0VBug?t=171), [420s](https://youtu.be/5w3s2T0VBug?t=418) and [640s](https://youtu.be/5w3s2T0VBug?t=642) for the studio cut, and [200s](https://youtu.be/MBrqmlmh_40?t=201), [450s](https://youtu.be/MBrqmlmh_40?t=452) and [690s](https://youtu.be/MBrqmlmh_40?t=686) for the live cut. I especially find the respective last sections to be very interesting, as it beautifully shows the _allargando_ right before the song's grand finale. The _allargando_ for the live cut is a lot harder to see, but that is because it is a lot more drastic with even a little pause at the end. This makes the finale (which is a bit longer than the studio cut too) even more impactful.

Conclusion
===

I have made a hierarchical clustering of Rush's studio albums, to see whether I can divide it's discography up in different eras. The clusters that came out were very interesting and sometimes surprising, and gave me an even better understanding of the characteristics of Rush. It's interesting to see that most clusters contain multiple albums that were released consecutively, but most of clusters also feature some albums that were released in a very different time period. Overall I must say that all clusters were very accurate, and I think inclusion of the timbre and chroma features, deltas and deltadeltas and the mean interonset interval contributed a lot to the final clustering. 
